...
Phase 1 (Manual Provisioning): Initial deployment on a bare-metal Amazon Linux instance, configuring Python, Git, and PostgreSQL manually.
Phase 2 (Containerization): Authored a Dockerfile to package the application, ensuring consistency across local and cloud environments.
Phase 3 (Database Decoupling): Migrated the local PostgreSQL database to a fully managed AWS RDS instance inside a private subnet.
Phase 4 (Storage Decoupling): Transitioned avatar file storage from the local EC2 filesystem to Amazon S3.
Phase 5 (High Availability): Placed the compute instances behind an Elastic Load Balancer.


🔐 Security Best Practices Implemented

- IAM Roles over Credentials: The application accesses Amazon S3 via an attached EC2 IAM Role (AmazonS3FullAccess scoped down to specific buckets) rather than hardcoding AWS Access Keys in environment variables.
- Network Isolation: The RDS database is placed in a private subnet and is only accessible from the EC2 instance's specific Security Group.
- AWS SSO: Infrastructure management and CLI interactions are authenticated securely using AWS Single Sign-On (SSO) rather than legacy IAM users.
...
- An AWS Account and AWS CLI configured via aws configure sso.
- Docker installed locally for image building.
- A GitHub account to fork the repository.
- Basic familiarity with SSH and Linux command-line tools.
